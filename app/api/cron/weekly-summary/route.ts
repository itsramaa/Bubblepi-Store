...
export async function GET(request: NextRequest) {
  ...
  const [orders, topProducts] = await Promise.all([
    db.order.findMany({ where: { createdAt: { gte: since } }, select: { status: true, total: true, guestEmail: true } }),
    db.orderItem.groupBy({
      by: ["variantId"],
      where: { order: { status: "DELIVERED", paidAt: { gte: since } } },
      _sum: { quantity: true },
  ...
  const uniqueBuyers = new Set(fulfilled.map((o) => o.guestEmail)).size
