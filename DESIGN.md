...
Bubblepi Store is the canonical example of a generous, photography-led consumer marketplace. The base canvas is **pure white** (`{colors.canvas}` — #ffffff) with deep near-black ink (`{colors.ink}` — #222222) for headlines and body, and a single voltage of **Bubblepi Pink** (`{colors.primary}` — #F4ABC4) carrying every primary CTA, the search-button orb, the heart save state, and inline brand links. There is no secondary brand color in mainline marketing — the **Purple** (`{colors.luxe}` — #595B83) and **Navy** (`{colors.plus}` — #333456) tokens are sub-brand accents that only appear inside specific contexts.

Type runs **Airbnb Cereal VF** (a custom variable font), with **Circular** as the historic in-house fallback and a system stack underneath. Cereal sits at modest weights — display headlines render at 22–28px in weight 500–600, not the heavy 700+ weights that financial or enterprise systems lean on. The hero h1 on the homepage is just 28px / 700, which would feel small on a typical SaaS page; here it works because the layout leans on photography (city collage, property cards) for visual weight rather than typographic muscle.
...
- Single accent color: `{colors.primary}` (#F4ABC4 — "Bubblepi Pink") carries every primary CTA, the search orb, the heart save state, and the brand wordmark. Used scarcely — most pages are 90% white + ink with one or two pink moments.
...
- Pill-shaped global search bar: white surface, fully rounded (`{rounded.full}`), divided by 1px hairlines into Where / When / Who segments, terminated by a circular Bubblepi pink search orb (`{component.search-orb}`).
...
- **Bubblepi Pink** (`{colors.primary}` — #F4ABC4): The single brand color. Used for primary CTA backgrounds (Reserve, Continue), the search orb, the heart save state on property cards, and inline brand links.
- **Bubblepi Pink Active** (`{colors.primary-active}` — #D48CA5): The press / pointer-down variant — slightly more saturated. Used on `{component.button-primary-active}`.
- **Bubblepi Pink Disabled** (`{colors.primary-disabled}` — #FAD5E2): A pale tint used on disabled CTAs.
- **Purple** (`{colors.luxe}` — #595B83): Sub-brand accent. Only appears inside specific contexts — never in mainline marketing.
- **Navy** (`{colors.plus}` — #333456): Sub-brand accent. Same scoping as Purple — sub-product only.

### Surface

- **Canvas** (`{colors.canvas}` — #ffffff): The default page floor for every public page. Bubblepi does not have a dark mode on the public web.
...
- **Star Rating** (`{colors.star-rating}` — #222222): The same ink token — star icon and "4.81" rating numbers all render in ink rather than a yellow/gold color, which is a deliberate brand choice (yellow stars feel cheap in travel context).
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on pink CTAs.

### Semantic

- **Error** (`{colors.primary-error-text}` — #c13515): Inline error text for form validation. Distinct from Bubblepi Pink — slightly darker, more saturated red.
...
- **Scrim** (`{colors.scrim}` — #000000 at 50% opacity): The global modal backdrop tone — login dialog, language picker. Stored as the base hex; opacity is applied at render time.
...
- **Card hover float:** `box-shadow: rgba(0, 0, 0, 0.02) 0 0 0 1px, rgba(0, 0, 0, 0.04) 0 2px 6px 0, rgba(0, 0, 0, 0.1) 0 4px 8px 0` — applied to property cards on pointer hover, the search bar at rest, and the dropdown menus. This is the single shadow definition in the entire system.
...
**`button-primary`** — Pink fill, white text, 8px radius, 14×24px padding, 48px height, weight 500. The most common CTA across the system.

**`button-primary-active`** — The press state. Background flips to `{colors.primary-active}`. No transform, no shadow change.

**`button-primary-disabled`** — Pale pink tint at #FAD5E2 with white text. Cursor not-allowed.

**`button-secondary`** — White fill with ink text and a 1px ink outline. 8px radius. Used for "Save", "Cancel", and inverse CTAs over pink surfaces.
...
**`button-pill-rausch`** — A pill-shaped pink CTA used on featured cells — 9999px radius, 10×20px padding, 14px label.
...
**`search-orb`** — The circular pink orb terminating the right edge of the search bar. 48×48px, fully rounded, white magnifying-glass icon centered. The hottest single color moment on the homepage.

### Top Navigation

**`top-nav`** — White surface, 80px height, 1px bottom hairline. The Bubblepi wordmark sits flush left, the three product tabs sit in the dead center, and account utilities sit flush right.
...
**`new-tag`** — A tiny rounded-pill badge (`{rounded.full}`) anchored top-right of an icon, carrying the uppercase "NEW" label in `{typography.uppercase-tag}` (8px / 700 with 0.32px tracking, uppercase).

### Listing Cards

**`property-card`** — A photo-first card. 1:1 aspect-ratio image with `{rounded.md}` corner clipping, image carousel dots overlay, "Guest favorite" floating badge top-left (`{component.guest-favorite-badge}`), and a heart icon top-right (`{component.icon-button-circle}` in default outlined state, pink-filled when saved). Beneath the image: 4–5 lines of meta — title (`{typography.title-md}`), distance / dates (`{typography.body-sm}` muted), and price right-aligned.

**`property-card-photo`** — The photo plate itself, separated as a token because some surfaces reuse just the photo without the meta block.
...
**`rating-display-card`** — The signature listing-detail moment. A 64px / 700 rating number flanked left and right by tiny laurel-wreath SVG ornaments. Beneath the rating: "Guest favorite" tagline and a row of ink stat columns. The largest typographic weight in the whole system.
...
**`host-card`** — A white card with `{rounded.md}` rounding and 24px padding holding a host avatar, name, response-rate stat, and a "Contact host" `{component.button-secondary}`.
...
**`footer-light`** — White surface (matches the page canvas), 48×80px padding. Three columns of link blocks, separated by generous 24px gutters. Each column heads with a `{typography.title-sm}` ink label and stacks `{component.footer-link}` rows in `{typography.body-sm}` ink.

**`legal-band`** — A bottom strip beneath the footer columns carrying the copyright line, language picker (globe icon), currency picker, and social icons. All text in muted `{colors.muted}` at `{typography.caption-sm}`.
...
- **Hover state colors:** intentionally not documented per the global no-hover policy — the actual `:hover` styling for property cards is a subtle elevation lift, but precise extraction is unreliable.
- **Loading states / skeleton screens:** not visible on the extracted surfaces.
- **Map view styling:** the search-results map uses Mapbox-tinted tiles with custom pink markers; not captured here.
...
- **Sub-brand palettes:** Purple (`{colors.luxe}`) and Navy (`{colors.plus}`) are documented as tokens, but their full sub-system (typography overrides, surface treatment) lives on separate sub-domains and is not captured here.